  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,paddingBottom:80}}>
      ...
      <div style={{padding:"0 20px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <p style={{color:C.purple,fontSize:13,fontWeight:600}}>✨ RecipeRun Preview</p>
        <p style={{color:C.muted,fontSize:12,marginTop:3}}>Paste or type a recipe to extract ingredients. Photo + URL import are fully active in the deployed app.</p>
      </div>
      {error&&<div style={{background:C.danger+"18",border:`1px solid ${C.danger}44`,borderRadius:14,padding:"12px 16px"}}><p style={{color:C.danger,fontSize:14}}>⚠ {error}</p></div>}
        ...
        <Card>
          <p style={{color:C.muted,fontSize:11,fontWeight:700,letterSpacing:".08em",marginBottom:8}}>PASTE A RECIPE LINK</p>
          <p style={{color:C.muted,fontSize:12,marginBottom:10}}>AllRecipes, Food Network, TikTok, Instagram, YouTube, NYT Cooking and more</p>
          <div style={{display:"flex",gap:8}}>
            <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://www.allrecipes.com/recipe/…"
              style={{flex:1,background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",color:C.text,fontSize:14,fontFamily:"inherit",outline:"none"}}/>
            <Btn onClick={()=>url&&run(`The user wants to cook a recipe from this URL: ${url}. Since you cannot fetch URLs, create a plausible full ingredient list based on the URL path (e.g. tuscan-chicken → Tuscan Garlic Chicken). Return a complete realistic recipe.`,"text")} disabled={!url} style={{padding:"12px 16px",borderRadius:12,flexShrink:0}}>→</Btn>
          </div>
        </Card>

        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[
            {label:"Photo",emoji:"📸",sub:"Scan a recipe card",ref:fileRef},
            {label:"Screenshot",emoji:"🖼️",sub:"Upload an image",ref:fileRef2},
          ].map(x=>(
            <Card key={x.label} onClick={()=>x.ref.current?.click()} style={{textAlign:"center",padding:"24px 12px",cursor:"pointer"}}>
              <input ref={x.ref} type="file" accept="image/*" onChange={handleFile} style={{display:"none"}}/>
              <div style={{fontSize:32,marginBottom:8}}>{x.emoji}</div>
              <p style={{color:C.text,fontWeight:700,fontSize:14}}>{x.label}</p>
              <p style={{color:C.muted,fontSize:12,marginTop:2}}>{x.sub}</p>
            </Card>
          ))}
        </div>
    ...
    <div style={{minHeight:"100vh",background:C.bg,paddingBottom:80}}>
      ...
      <div style={{padding:"0 20px 16px"}}>
        <button onClick={()=>scanRef.current?.click()} disabled={scanning} style={{
          width:"100%",padding:"20px 16px",borderRadius:20,cursor:"pointer",fontFamily:"inherit",
          background:`linear-gradient(135deg,${C.accent}22,${C.accentDim}44)`,
          border:`1.5px dashed ${C.accent}88`,
          display:"flex",alignItems:"center",gap:16,
        }}>
          <div style={{width:52,height:52,borderRadius:16,background:C.accent,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0}}>
            {scanning?"⏳":"📷"}
          </div>
          <div style={{textAlign:"left"}}>
            <p style={{color:C.accent,fontWeight:800,fontSize:16,fontFamily:"inherit"}}>
              {scanning?"Scanning…":"Scan My Fridge"}
            </p>
            <p style={{color:C.muted,fontSize:13,marginTop:2,fontFamily:"inherit"}}>
              {scanning?"AI is identifying your food items…":"Take a photo — AI spots everything inside"}
            </p>
          </div>
        </button>
      </div>
